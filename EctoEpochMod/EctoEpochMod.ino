...
Version 1.0
 ...
 EctoEpochMod Created by Josh Ward
 */
...
//None of this is used for the EctoEpochMod
//Leaving in case we want to use it later
...
void loop()
{
  static unsigned long _timeRef = 0;
  static uint8_t _step = 0;

  if(TimeElapsed(_timeRef, 50))
  {
    _timeRef = millis(); //IMPORTANT!!! This resets the timeout so things happen when they should.

    dispBuf = vals[_step]; // push data to display buffer

    _step++; //increment for next time through loop
    if(_step >= 17) 
      _step = 0; 
    //reset to 0 if need be
  }
}
